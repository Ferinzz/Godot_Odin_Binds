...
EditorToaster_MethodBind_List :: struct {
  push_toast: struct{
    ...
    m_call: proc "c" (_:^GDW.MethodBind, obj: EditorToaster, #by_ptr args: struct{message: ^GDW.gdstring, severity: ^EditorToaster_Severity, tooltip: ^GDW.gdstring, }, r_ret: rawptr = nil)
  },
  };
EditorToaster_Init_ :: proc (EditorToaster_methods: ^EditorToaster_MethodBind_List, loc := #caller_location) {
  MB_ptr_call:=gdAPI.get_Interface_Address("object_method_bind_ptrcall")
  EditorToaster_methods.push_toast._push_toast = (cast(^GDW.MethodBind)classDBGetMethodBind3(.EditorToaster, "push_toast", 1813923476, loc))
