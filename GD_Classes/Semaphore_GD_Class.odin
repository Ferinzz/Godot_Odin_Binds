...
Semaphore_Init_ :: proc (Semaphore_methods: ^Semaphore_MethodBind_List, loc := #caller_location) {
  ...
  Semaphore_methods.wait.m_call = cast(type_of(Semaphore_methods.wait.m_call))gdAPI.get_Interface_Address("object_method_bind_ptrcall")
  Semaphore_methods.try_wait._try_wait = (cast(^GDW.MethodBind)GDW.classDBGetMethodBind3(.Semaphore, "try_wait", 2240911060, loc))
  Semaphore_methods.try_wait.m_call = cast(type_of(Semaphore_methods.try_wait.m_call))gdAPI.get_Interface_Address("object_method_bind_ptrcall")
  Semaphore_methods.post._post = (cast(^GDW.MethodBind)GDW.classDBGetMethodBind3(.Semaphore, "post", 1667783136, loc))
  Semaphore_methods.post.m_call = cast(type_of(Semaphore_methods.post.m_call))gdAPI.get_Interface_Address("object_method_bind_ptrcall")
};
