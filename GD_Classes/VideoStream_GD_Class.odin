...
VideoStream_Init_ :: proc (VideoStream_methods: ^VideoStream_MethodBind_List, loc := #caller_location) {
  ...
  VideoStream_methods.set_file.m_call = cast(type_of(VideoStream_methods.set_file.m_call))gdAPI.get_Interface_Address("object_method_bind_ptrcall")
  VideoStream_methods.get_file._get_file = (cast(^GDW.MethodBind)GDW.classDBGetMethodBind3(.VideoStream, "get_file", 2841200299, loc))
  VideoStream_methods.get_file.m_call = cast(type_of(VideoStream_methods.get_file.m_call))gdAPI.get_Interface_Address("object_method_bind_ptrcall")
};
